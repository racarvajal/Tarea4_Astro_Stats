...
# Simulate multiple draws from polynomial model
...
# Polynomial model
# Y_k = M_pol.Theta_pol + L_pol
Theta_pol = Theta[1:]  # Polynomial coefficients
M_pol = M[:, 1:]  # Design matrix
L_pol = L[1:]  # 1s vector
...
cdf_X_sq_pol = [chi2.cdf(np.shape(datos)[0] - (p + 1), X_sq_new[:][i]) for i in np.arange(0, int(n_draws))]
p_value_pol = np.ones(n_draws) - cdf_X_sq_pol

# Plot histogram for p-values
num_bins = 10
# n, bins, patches = plt.hist(p_value_pol, num_bins, histtype='step', color='grey')
n, bins, patches = plt.hist(p_value_pol, num_bins, histtype='bar', color='grey', label=('$n=%i$' % n_draws))
plt.xlabel('$\mathrm{p-value}$')
plt.ylabel('$\mathrm{Frequency}$')
plt.legend(loc='best')
# plt.savefig('T4_p2_a.pdf')
plt.show()
